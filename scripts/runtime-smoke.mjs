...
const profile=join(process.cwd(),".runtime-smoke-profile");
...
if(!wsUrl){
  browser.kill("SIGKILL");server.close();
...
await command("Runtime.evaluate",{expression:"window.backrooms.beginIntroReveal()",returnByValue:true});
await delay(1500);
...
if(!state.running||state.paused||state.introActive||state.runtimeFaulted)throw new Error("Gameplay did not remain healthy after start: "+JSON.stringify(state));
